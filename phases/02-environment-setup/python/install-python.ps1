# strangeloop Setup - Python Installation Router
# Version: 2.0.0
# Routes to platform-specific Python installation implementation
...
try {
    Write-Info "Python Installation Router - detecting platform..."
    ...
    if ($platformIsWindows) {
        $targetScript = Join-Path $PSScriptRoot "windows\install-python-windows.ps1"
        Write-Info "Routing to Windows Python installation"
    } elseif ($platformIsWSL -or $platformIsLinux) {
        $targetScript = Join-Path $PSScriptRoot "linux\install-python-linux.ps1"
        Write-Info "Routing to Linux Python installation"
    } else {
    ...
    # Return the result
    if ($exitCode -eq 0) {
        Write-Success "Python installation routing completed successfully"
        exit 0
    } else {
        Write-Error "Python installation failed with exit code: $exitCode"
        exit $exitCode
    }
    
} catch {
    Write-Error "Python installation router failed: $($_.Exception.Message)"
