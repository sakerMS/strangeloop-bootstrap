    ...
    try {
        ...
        if ($WhatIf) {
            ...
            if ($platformIsWindows -and -not $NoWSL) {
                ...
                Write-Host "what if:      wsl -d Ubuntu-24.04 pwsh <dynamically-resolved-path>/setup-environment-linux.ps1" -ForegroundColor Yellow
                Write-Host ""
        ...
        try {
            ...
            $routerPath = Join-Path $PSScriptRoot "orchestration\setup-environment-router.ps1"
